  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: theme.modalOverlay }}>
      <div style={{ background: theme.surface, borderRadius: "18px", padding: "24px", width: "100%", maxWidth: "380px", margin: "0 16px", border: `1px solid ${theme.border}`, transition: "background 0.25s" }}>
          ...
          {allFolders.map(folder => {
            ...
            return (
              <button
                ...
                style={{ background: isSelected ? color : theme.surfaceAlt, color: isSelected ? "white" : theme.textMed, border: `1px solid ${isSelected ? "transparent" : theme.border}`, padding: "5px 13px", borderRadius: "20px", fontSize: "12px", fontWeight: "600", fontFamily: "inherit", cursor: "pointer" }}
              >
                {folder.name}
              </button>
            );
        ...
        <button
          onClick={() => { onConfirm(tempCategories); onClose(); }}
  ...
  return (
    <div style={{ display: "flex", gap: "2px" }} onMouseLeave={() => setHovered(null)}>
      {[1, 2, 3, 4, 5].map(star => {
        ...
        return (
          <div key={star} onMouseMove={e => handleMouseMove(e, star)} onClick={e => handleClick(e, star)} style={{ cursor: "pointer" }}>
  ...
  if (photos.length === 0) {
    return (
      <div
        ...
        style={{ border: `1.5px dashed ${theme.border}`, borderRadius: "10px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "6px", padding: "28px 20px", cursor: "pointer", background: theme.surfaceAlt, transition: "border-color 0.15s, background 0.15s" }}
    ...
    <div>
      <div style={{ position: "relative", borderRadius: "10px", overflow: "hidden", background: "#111", aspectRatio: "4/3" }}>
        ...
        <div style={{ position: "absolute", bottom: "10px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "4px", zIndex: 2 }}>
          {photos.map((_, i) => (
            <div key={i} onClick={() => { if (!animating) setCurrentIndex(i); }} style={{ width: i === currentIndex ? "16px" : "5px", height: "5px", borderRadius: "3px", background: i === currentIndex ? "white" : "rgba(255,255,255,0.45)", cursor: "pointer", transition: "all 0.2s" }} />
          ))}
        </div>
...
export default function PostDetail({ onBack, data, setData, allFolders, theme }) {
  ...
  const inputStyle = (focused) => ({
    ...
    background: theme.inputBg, border: `1px solid ${focused ? theme.text : theme.border}`,
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, fontFamily: "'DM Sans', -apple-system, sans-serif", transition: "background 0.25s" }}>
      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "28px 24px 80px" }}>
        ...
        <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px", color: theme.textMuted, fontFamily: "inherit", fontSize: "11px", fontWeight: "700", marginBottom: "24px", padding: 0, textTransform: "uppercase", letterSpacing: "0.06em" }}>
        ...
        <div style={{ display: "flex", gap: "36px", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {isEditing ? (
              ...
              <h2 style={{ fontFamily: "inherit", fontSize: "26px", fontWeight: "800", color: theme.text, margin: "0 0 14px", letterSpacing: "-0.5px", lineHeight: 1.2 }}>
            ...
            <div style={{ display: "flex", gap: "6px", marginBottom: "18px" }}>
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: "5px", background: isEditing ? theme.text : theme.surface, color: isEditing ? theme.bg : theme.textMed, border: `1px solid ${theme.border}`, padding: "7px 13px", borderRadius: "9px", fontSize: "12px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit" }}
              >
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: "5px", background: isFavourited ? "#FFF1F2" : theme.surface, color: "#EF4444", border: "1px solid #FECACA", padding: "7px 13px", borderRadius: "9px", fontSize: "12px", fontWeight: "600", cursor: "pointer", fontFamily: "inherit" }}
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill={isFavourited ? "#EF4444" : "none"} stroke="#EF4444" strokeWidth="2">
          ...
          <div style={{ width: "400px", flexShrink: 0 }}>
            <div style={{ background: theme.surface, borderRadius: "14px", overflow: "hidden", border: `1px solid ${theme.border}`, transition: "background 0.25s, border-color 0.25s" }}>
              ...
              <div style={{ position: "relative", background: "#000", height: `${embedHeight}px`, overflow: "hidden" }}>
                {embedUrl ? (
                  <iframe src={embedUrl} allowFullScreen scrolling="no" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture"
                    style={{ border: "none", position: "absolute", top: 0, left: 0, width: "100%", height: isTikTok ? "739px" : "100%", display: "block", colorScheme: "dark", background: "#000" }} />
                ) : (
              ...
              <div style={{ padding: "11px 16px", background: theme.surfaceAlt, borderTop: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", transition: "background 0.25s" }}>
                ...
                <a
                  href={data.externalUrls?.tiktok || data.externalUrls?.insta || "#"}
                  target="_blank"
